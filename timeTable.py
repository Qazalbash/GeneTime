...
class TimeTable(SelectionSchemes):
    ...
    def __init__(self, filename, populationSize, offspringsNumber, mutationRate, filenameStudents) -> None:
        super().__init__()
    ...
    def initializeChromosome(self):
        chromosome = {day: {room: [] for room in self.data.room_list} for day in self.availableDays}
        # Initialize faculty working hours for each day
        faculty_working_hours = {instructor: {day: [] for day in self.availableDays} for instructor in
                                 self.data.instructor_list}
        # Initialize student schedules for every student for each day
        studentSchedules = {student: {day: [] for day in self.availableDays} for student in self.data.studentList}
    ...
    @staticmethod
    def addToFacultySchedule(instructor, day, start_time, end_time, classNumber, faculty_working_hours):
    ...
    def addToStudentSchedules(self, classNumber, day, start_time, end_time, studentSchedules):
        if classNumber not in self.data.classToStudentDict:
            ...
            for classNumber, data in self.data.class_nbr_dict.items():
                ...
                for day in assigned_days:
                    ...
                    if not self.facultyClash(data["Instructor"], day, current_class_start_time, end_time,
                        ...
                        numberOfRoomsInList = len(self.data.room_list)
                        while True:
                            room_number_index = (room_number_index + 1) % numberOfRoomsInList
    ...
    def fitnessEvaluation(self, chromosome, C1, studentSchedules):
        ...
        # a, b, c, d = random.choices([a, b, c, d], k=4)
        # totalFitness = a * fitness_same_room + b * fitness_same_time + c * fitness_withinlimit + d * fitness_free_slot
    ...
    @staticmethod
    def generate_time(startTime, duration):
        endTime = (
    ...
    @staticmethod
    def SOFT_class_in_same_room(C1):
        same_room_counter = 0
    ...
    @staticmethod
    def SOFT_class_at_same_time(C1):
        same_time_counter = 0
        for class_number, instances in C1.Class_Dict.items():
            flag = True
            if len(instances) > 1:
    ...
    @staticmethod
    def SOFT_student_conflicts(studentSchedules):
    ...
    @staticmethod
    def updateFacultySchedule(instructor, day1, number, day2, start, end, faculty_working_hours):
    ...
    @staticmethod
    def updateClassDetails(number, day1, room1, start, end, day2, room2, classDetails):
